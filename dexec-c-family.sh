...
function dexec_build() {
    if dexec_array_contains "${dexec_default_output_flag}" "${dexec_build_args[@]}"; then
        dexec_build_name=$(dexec_array_item_after "${dexec_default_output_flag}" ${dexec_build_args[@]})
    else
        dexec_build_args+=(${dexec_default_output_flag})
        dexec_build_args+=(${dexec_build_name})
    fi
